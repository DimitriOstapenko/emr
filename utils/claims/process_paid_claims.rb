...
def found_by_hc_and_date?(pat,visit)
     ...
     claims.each do |c|
       if c.date == visit.entry_ts.to_date 
         visit.update_attributes(:status => PAID, :amount => c.damt_paid, :claim_id => c.id, :export_file => c.accounting_no)
         c.update_attribute(:visit_id, visit.id)
	 puts "updated: pat: #{pat.id} visit: #{visit.id} date: #{c.date}"
	 return true
       end
     end
     puts "Couldn't find in claims by HC/date either"
     return false
end
...
# This will cover all HCP containing visits and will exclude all CSH, INV, IFH and PRV claims	
   ...
   next unless [OHIP_PATIENT,RMB_PATIENT].include?(pat.pat_type)

   if v.export_file.present? && v.export_file.match(/^\w{8}/)
      if cl = Claim.find_by(accounting_no: v.export_file)
	 ...
	 no_claim_count += 1
       ...
       good_claims_count += 1
       puts "Pat #{v.patient_id}, Visit #{v.id} acc_ref: (#{v.export_file}) not found in claims table, but claim for this HC/date was paid by MOH. Fixed attributes."
   else 	     
     no_acc_ref_count += 1
