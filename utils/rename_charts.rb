...
#   
# NB! Disable uploader in charts before running! also, change method chart.filespec to use self.chart
...
#  File.rename( chart.filespec, new_chart_pathname) rescue nil
  ...
  chart.save!
  puts "id: #{chart.id} filename: #{chart.patient.chart_filename} chart: #{chart.patient.id}.pdf "
end
