...
#  puts row.to_hash
  ...
  unless Patient.exists?(patient_id)
    puts "Patient not found: #{patient_id} for bill #{bill_id}" 
    next
  end 

  visit_id = row['ordno']
#  unless Visit.exists?(visit_id)
#    puts "Visit not found: #{visit_id} for bill #{bill_id}"
#    next
#  end
  ...
  if billing.save
     puts "*** #{billing.id}  saved"
  else
     puts "Problem billing: #{billing.id}"
...
puts "#{Billing.count} records added to billings table "
